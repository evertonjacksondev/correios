...
const updatePriceFreight = async (db, userToken) => {
  try {
    ...
    for (let sigep of contractSigepWeb) {
      ...
      let { cepOrigin, senha, administrador, ufOrigin } = sigep;
      ...
      for (let zip of zipCode) {
        ...
        for (let weith of fieldsWeith) {
          ...
          for (let servico of cServico) {
            try {
              if (servico == '03220' && UF == ufOrigin || UF != ufOrigin && servico == '03298') {
                let params = {
                ...
                let update = {
                  $set: {
                    UF, ufOrigin, zipStart, zipEnd, servicoEntrega: servico, prazoEntrega: Number(PrazoEntrega), weithEnd: weith, valor: Valor, createdAt: new Date(), msgErro: MsgErro
                  }
                };
